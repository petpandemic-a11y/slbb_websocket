...
// HARD SKIP: if any log line contains 'remove'
const hasRemove = logMessages.some(l => l && l.toLowerCase().includes('remove'));
if (hasRemove) {
    logger.debug(`[SKIP] 'remove' keyword in logs: ${signature.slice(0, 8)}...`);
    return;
}
...
// CRITICAL: Skip swap transactions immediately
    ...
    detectBurnPattern(logs) {
        ...
        for (const log of logs) {
            // HARD SKIP: any 'remove' in logs
            if ((log || '').toLowerCase().includes('remove')) return false;
    ...
    isRemoveLiquidityTx(tx) {
        try {
            ...
            // HARD policy: any 'remove' in logs means remove-liquidity
    ...
    async analyzeTransaction(signature, slot) {
        try {
            ...
            // HARD SKIP: 'remove' keyword in logs at analyze stage
            const _logsLower = (tx.meta.logMessages || []).join(' ').toLowerCase();
            if (_logsLower.includes('remove')) {
